...
class Controller {
  ...
  redirectInactive() {
    ...
    setInterval(() => {
      ...
      if (currentTime > (lastTime + 30000 * 2)) {
        window.location.href = `${room}/${currentDate}`;
      }
    ...
    setInterval(() => {
      const timeDiff = new Date().getTime() - this.lastLocalInsertTime;

      if (timeDiff > 30000 && timeDiff < 45000)  {
  ...
  updateSaveStatus(doc=document) {
      BackendHelper.getPage(DateHelper.currentDate()).then((result) => {
