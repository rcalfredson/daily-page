...
class Controller {
  ...
  backupChanges() {
    ...
    if ((now - this.lastBackup) > 5000) {
      setTimeout(() => this.backendHelper.syncPage(this.editor.currentContents()), 2000);
      this.lastBackup = now;
    }
  }
  ...
  removeFromNetwork(peerId, doc=document) {
    ...
    if (idx >= 0) {
      ...
      if (this.firstPeerId() == this.myPeerId()) {
        this.backendHelper.removePeer(peerId);
      }
    }
  }
...
// // purely for mock testing purposes
  ...
  findNewTarget() {
    this.backendHelper.getPeers().then(result => {
