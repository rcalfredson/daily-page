...
class Controller {
  ...
  updateSaveStatus(action, id, doc=document) {
    if (!(id !== this.backup.lastMessageID || id === 'local')) {
        return;
    }
    if (action.includes('begin')) {
      doc.getElementById('sync-infobox').innerText = 'Saving...';
    } else {
      setTimeout(() => {
        if ((new Date().getTime() - this.backup.lastUpdated) > 2000) {
          doc.getElementById('sync-infobox').innerText = action.includes('end') ? 'Saved.' :
            'Save error!';
          this.backup.pendingRequests = 0;
        }
      }, 2000);
    }
    if (id !== this.backup.lastMessageID && id !== 'local') {
      this.backup.updateMessageID(id);
      this.backup.lastUpdated = new Date().getTime();
      this.broadcast.sendSaveEvent(action, id);
    }
  }
  ...
  backupChanges() {
    ...
    if ((now - this.backup.lastUpdated) > 1000 && this.backup.pendingRequests < 1) {
      this.backup.updateMessageID();
      this.broadcast.beginSave(this.backup.lastMessageID);
      this.updateSaveStatus('beginSave', 'local');
      setTimeout(() => {
        BackendHelper.syncPage(this.editor.currentContents()).then(() => {
          this.backup.updateMessageID();
          this.broadcast.endSave(this.backup.lastMessageID);
          this.updateSaveStatus('endSave', 'local');
        }).catch(() => {
          this.backup.updateMessageID();
          this.broadcast.failSave(this.backup.lastMessageID);
          this.updateSaveStatus('failSave', 'local');
        });
      }, 500);
      this.backup.lastUpdated = now;
      this.backup.pendingRequests++;
    }
  }
...
// // purely for mock testing purposes
  ...
  findNewTarget() {
    BackendHelper.getPeers().then(result => {
      const possibleTargets = result.ids.filter(id => id !== this.myPeerId());
