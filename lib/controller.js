...
class Controller {
  ...
  updateSaveStatus(action, doc=document) {
    if (action.includes('begin')) {
      doc.getElementById('sync-infobox').innerText = 'Saving...';
    } else {
      setTimeout(() => {
        if ((new Date().getTime() - this.lastBackup) > 2000) {
          doc.getElementById('sync-infobox').innerText = action.includes('end') ? 'Save succeeded.' :
  ...
  backupChanges() {
    ...
    if ((now - this.lastBackup) > 1000) {
      this.broadcast.beginSave();
      setTimeout(() => {
        BackendHelper.syncPage(this.editor.currentContents()).then(() => {
          this.broadcast.endSave();
        }).catch(() => {
          this.broadcast.failSave();
        });
      }, 500);
      this.lastBackup = now;
