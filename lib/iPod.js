...
$(document).ready(async () => {
  ...
  var htmlString = function(inputString) {
...
if($('.select').attr('href') == "#audioplayer")
  ...
  async function loadSongsMenu() {
    ...
    Songs.forEach((song) => {
      ...
      songLink.id = `${htmlString(song.album.name)}-${htmlString(song.id)}-link`;
  ...
  async function loadAlbumsMenu() {
    ...
    Albums.forEach((album) => {
      promises.push((async () => {
        ...
        albumLink.id = `${htmlString(album.name)}-${htmlString(album.artist)}-link`;
        albumLink.href = `#${htmlString(album.name)}-${htmlString(album.artist)}`;
        ...
        songTile.id = `${htmlString(album.name)}-${htmlString(album.artist)}`;
        ...
        songList.id = `${htmlString(album.name)}-${htmlString(album.artist)}-list`;
        albumData.tracks.forEach((song) => {
          const songLink = document.createElement('a');
          songLink.id = `${htmlString(album.name)}-${htmlString(song.id)}-link`;
          songLink.href = '#audioplayer';
    ...
    Albums.forEach(album => {
      const tracks = document.getElementById(`${htmlString(album.name)}-${htmlString(album.artist)}-list`)
  ...
  async function loadArtistsMenu() {
    ...
    Artists.forEach((artist) => {
      promises.push((async () => {
        ...
        artistLink.id = `${htmlString(artist)}-link`;
        artistLink.href = `#${htmlString(artist)}`;
        ...
        albumsTile.id = htmlString(artist);
        ...
        albumsList.id = `${htmlString(artist)}-list`;
        albums.forEach((album) => {
          const albumLink = document.createElement('a');
          albumLink.id = `${artist}-${album}-link`;
          albumLink.href = `#${htmlString(album)}-${htmlString(artist)}`;
    ...
    Artists.forEach(artist => {
      const albums = document.getElementById(`${htmlString(artist)}-list`)
        .getElementsByTagName('a');
