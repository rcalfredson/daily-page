...
class Editor {
  ...
  trackRemainingTime() {
    setInterval(() => {
      ...
      const displayed = (DateHelper.closingTime() - now) <= 200 * 60 * 1000;

      timeLeftElement.innerText = DateHelper.roundedDuration(DateHelper.closingTime() - now);
      closingSoonInfoBox.style.display = displayed ? 'inline' : 'none';
    }, 1000);
  }
...
//          this.processInsert(changeObj);    // uncomment this line for palindromes!
  ...
  currentContents() {
    return [...document.getElementsByClassName('CodeMirror-line')]
      .map(line => {
        return [...line.firstChild.childNodes].filter(node => node.nodeName === '#text').map(node => node.textContent).join('');
      }).join('\n');
  }
