...
class Broadcast {
  ...
  beginSave(id) {
    this.sendSaveEvent('beginSave', id);
  }

  endSave(id) {
    this.sendSaveEvent('endSave', id);
  }

  failSave(id) {
    this.sendSaveEvent('failSave', id);
  }

  sendSaveEvent(type, id) {
    this.send({ type: type, id: id });
  }
  ...
  onData(connection) {
    connection.on('data', data => {
      ...
      switch(dataObj.type) {
        ...
        case 'endSave':
        case 'failSave':
          this.controller.updateSaveStatus(dataObj.type, dataObj.id);
          break;
        default:
