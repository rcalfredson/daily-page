...
class Broadcast {
  ...
  requestConnection(target, peerId, siteId) {
    ...
    // 🔑 Make outgoing connections behave like incoming ones
  ...
  acceptConnRequest(peerId, siteId) {
    ...
    this.onConnection(connBack);
    this.onData(connBack);
    this.onConnClose(connBack);
  ...
  onConnClose(connection) {
    connection.on('close', () => {
      console.log('[Broadcast] DataConnection closed:', connection.peer);
