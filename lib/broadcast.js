...
class Broadcast {
  ...
  beginSave() {
    this.send({ type: 'beginSave' });
    this.controller.updateSaveStatus('begin');
  }

  endSave() {
    this.send({ type: 'endSave' });
    this.controller.updateSaveStatus('end');
  }

  failSave() {
    this.send({ type: 'failSave' });
    this.controller.updateSaveStatus('fail');
  }
  ...
  onData(connection) {
    connection.on('data', data => {
      ...
      switch(dataObj.type) {
        ...
        case 'endSave':
        case 'failSave':
          this.controller.updateSaveStatus(dataObj.type);
          break;
        default:
