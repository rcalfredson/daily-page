...
export async function buildCommentNotificationEmail({
  ...
  const subject = t('notifications.email.blockComment.subject', {
    blockTitle: safeBlockTitle || t('notifications.email.blockComment.fallbackBlockTitle')
  });

  const heading = recipientUsername
    ? t('notifications.email.blockComment.headingWithName', { username: safeRecipient })
    : t('notifications.email.blockComment.heading');

  const html = `
    <h2>${heading}</h2>
    <p>${t('notifications.email.blockComment.body', {
      actorUsername: safeActor || t('notifications.email.blockComment.fallbackActor'),
      blockTitle: safeBlockTitle || t('notifications.email.blockComment.fallbackBlockTitle')
    })}</p>
    <blockquote style="margin: 12px 0; padding-left: 12px; border-left: 3px solid #ccc; color: #444;">
      ${safeExcerpt}
    </blockquote>
    <p><a href="${safeLink}">${t('notifications.email.blockComment.cta')}</a></p>
  `;
